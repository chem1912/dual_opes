...
template <class mode>
class OPESmetad_dual : public bias::Bias {
...
public:
  explicit OPESmetad_dual(const ActionOptions&);      //need to add new function
...
//bias_mode id: scheme1_convergence_convergence_part1 (s1ccp1)= 1
//         scheme1_convergence_convergence_part2 (s1ccp2)= 2
//         scheme1_convergence_exploration_V11 (s1cev1)= 3
//         scheme1_convergence_exploration_V12 (s1cev2)= 4
//         scheme1_convergence_exploration_V2 (s1cev2)= 5
//         scheme2_convergence_convergence_part1 (s2ccp1)= 6
//         scheme2_convergence_convergence_part2 (s2ccp2)= 7

struct s1ccp1 { static const int bias_mode=1; };
typedef OPESmetad_dual<s1ccp1> OPESmetad_s1ccp1;
PLUMED_REGISTER_ACTION(OPESmetad_s1ccp1,"OPES_METAD_SCHEME1_CONVERGENCE_CONVERGENCE_PART1")

struct s1ccp2 { static const int bias_mode=2; };
typedef OPESmetad_dual<s1ccp2> OPESmetad_s1ccp2;
PLUMED_REGISTER_ACTION(OPESmetad_s1ccp2,"OPES_METAD_SCHEME1_CONVERGENCE_CONVERGENCE_PART2")

struct s1cev11 { static const int bias_mode=3; };
typedef OPESmetad_dual<s1cev11> OPESmetad_s1cev11;
PLUMED_REGISTER_ACTION(OPESmetad_s1cev11,"OPES_METAD_SCHEME1_CONVERGENCE_EXPLORATION_V11")

struct s1cev12 { static const int bias_mode=4; };
typedef OPESmetad_dual<s1cev12> OPESmetad_s1cev12;
PLUMED_REGISTER_ACTION(OPESmetad_s1cev12,"OPES_METAD_SCHEME1_CONVERGENCE_EXPLORATION_V12")

struct s1cev2 { static const int bias_mode=5; };
typedef OPESmetad_dual<s1cev2> OPESmetad_s1cev2;
PLUMED_REGISTER_ACTION(OPESmetad_s1cev2,"OPES_METAD_SCHEME1_CONVERGENCE_EXPLORATION_V2")

struct s2ccp1 { static const int bias_mode=6; };
typedef OPESmetad_dual<s2ccp1> OPESmetad_s2ccp1;
PLUMED_REGISTER_ACTION(OPESmetad_s2ccp1,"OPES_METAD_SCHEME2_CONVERGENCE_CONVERGENCE_PART1")

struct s2ccp2 { static const int bias_mode=7; };
typedef OPESmetad_dual<s2ccp2> OPESmetad_s2ccp2;
PLUMED_REGISTER_ACTION(OPESmetad_s2ccp2,"OPES_METAD_SCHEME2_CONVERGENCE_CONVERGENCE_PART2")

template <class mode>
void OPESmetad_dual<mode>::registerKeywords(Keywords& keys)
{
...
//output components
  ...
  if(mode::bias_mode==2 || mode::bias_mode==4 || mode::bias_mode==7){
  ...
  if(mode::bias_mode==2){
    keys.addOutputComponent("s1ccp2V12","default","V12 in OPES_METAD_SCHEME1_CONVERGENCE_CONVERGENCE_PART2");
    keys.addOutputComponent("s1ccp2V2","default","V2 in OPES_METAD_SCHEME1_CONVERGENCE_CONVERGENCE_PART2");
  }
  if(mode::bias_mode==6){
    keys.addOutputComponent("s2ccp1V1","default","V1 in OPES_METAD_SCHEME2_CONVERGENCE_CONVERGENCE_PART1");
    keys.addOutputComponent("s2ccp1V21","default","V21 in OPES_METAD_SCHEME2_CONVERGENCE_CONVERGENCE_PART1");
  }
}

template <class mode>
OPESmetad_dual<mode>::OPESmetad_dual(const ActionOptions& ao)
  : PLUMED_BIAS_INIT(ao)
  , isFirstStep_(true)
  , counter_(1)
...
//other compulsory input
  ...
  switch(mode::bias_mode)
  {
    ...
    case 4:
        bias_prefactor_=-1;
        log.printf("  case 4 BIAS_PREFACTOR = %f\n",bias_prefactor_);
        break;
    case 5:
  ...
  if(getRestart())
  {
    ...
    if(ifile.FileExist(restartFileName))
    {
      ...
      else
      {
        ...
        KDEnorm_=mode::bias_mode==5?counter_:sum_weights_;
        if(!no_Zed_)
        {
          double sum_uprob=0;
...
//add and set output components
  ...
  if(mode::bias_mode==2){
    addComponent("s1ccp2V12");
    componentIsNotPeriodic("s1ccp2V12");
    getPntrToComponent("s1ccp2V12")->set(s1ccp2V12_);
    addComponent("s1ccp2V2");
    componentIsNotPeriodic("s1ccp2V2");
    getPntrToComponent("s1ccp2V2")->set(s1ccp2V2_);
  }
  if(mode::bias_mode==6){
    addComponent("s2ccp1V1");
    componentIsNotPeriodic("s2ccp1V1");
    getPntrToComponent("s2ccp1V1")->set(s2ccp1V1_);
    addComponent("s2ccp1V21");
    componentIsNotPeriodic("s2ccp1V21");
    getPntrToComponent("s2ccp1V21")->set(s2ccp1V21_);
  }
//printing some info
...
//set bias and forces
  ...
  if(mode::bias_mode==2){
    s1ccp2V12_=biasfactor_* bias;
    s1ccp2V2_=(1-biasfactor_)*bias;
  }
  if(mode::bias_mode==6){
      s2ccp1V1_=(1-1./biasfactor_)*bias;
      s2ccp1V21_=1./biasfactor_*bias;
  }
  if(mode::bias_mode==2){
    getPntrToComponent("s1ccp2V12")->set(s1ccp2V12_);
    getPntrToComponent("s1ccp2V2")->set(s1ccp2V2_);
...
template <class mode>
void OPESmetad_dual<mode>::update()
{
...
//do update
  if(getStep()%stride_==0 && (excluded_region_==NULL || excluded_region_->get()==0))
  {
    ...
    if(adaptive_sigma_)
    {
      const double factor=mode::bias_mode==5?1:biasfactor_;
    ...
    if(!fixed_sigma_)
    {
      const double size=mode::bias_mode==5?counter_:neff; //for EXPLORE neff is not relevant
...
template <class mode>
double OPESmetad_dual<mode>::getProbAndDerivatives(const std::vector<double>& cv,std::vector<double>& der_prob)
{
  double prob=0.0;
  if(!nlist_)
  {
...
template <class mode>
void OPESmetad_dual<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma)
{
  bool no_match=true;
  if(threshold2_!=0)
  {
...
template <class mode>
void OPESmetad_dual<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma,const double logweight)
{
  addKernel(height,center,sigma);
//write to file
...
template <class mode>
unsigned OPESmetad_dual<mode>::getMergeableKernel(const std::vector<double>& giver_center,const unsigned giver_k)
...
template <class mode>
void OPESmetad_dual<mode>::updateNlist(const std::vector<double>& new_center)
{
...
template <class mode>
void OPESmetad_dual<mode>::dumpStateToFile()
{
...
template <class mode>
inline double OPESmetad_dual<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x) const
...
template <class mode>
inline double OPESmetad_dual<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x, std::vector<double>& acc_der, std::vector<double>& dist)
{ //NB: cannot be a method of kernel class, because uses external variables (for cutoff)
  double norm2=0;
  for(unsigned i=0; i<ncv_; i++)
  {
...
template <class mode>
inline void OPESmetad_dual<mode>::mergeKernels(kernel& k1,const kernel& k2)
{
...
template <class mode>
void OPESmetad_dual<mode>::call_qmcode(bool& use_tot_pot,bool& store_wavefunction_,std::array<double, 2>& hamil) {
