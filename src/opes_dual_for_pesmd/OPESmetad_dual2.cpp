...
namespace PLMD {
  extern double total_bias_;
namespace opes { 
  extern std::array<double, 2>  hamiltonian_; //high and low-level hamiltonian [H0,H1]
  extern double deltaH0; //Inital Value of H1-H0
  extern bool deltaH0_isAssigned;
  extern double deltaH_; //H1-H0-deltaH0

  std::array<double, 2>  hamiltonian_; //high and low-level hamiltonian [H0,H1]
  double deltaH0;
  bool deltaH0_isAssigned=false;
  double deltaH_;

template <class mode>
class OPESmetad_dual2 : public bias::Bias {
...
private:
  ...
  double epsilon_;
  double sum_weights_;     // depends on bias
  double sum_weights2_;    // depends on bias

  bool no_Zed_;
  double Zed_;             // depends on bias
  double KDEnorm_;         // depends on bias
...
//kernels are truncated diagonal Gaussians
  struct kernel
  {
    ...
    kernel(double h, const std::vector<double>& c,const std::vector<double>& s):   //constructor
...
//neighbour list stuff
  ...
  bool calc_work_;      
  double work_;         // depends on bias
  double old_KDEnorm_;      // depends on bias
  std::vector<kernel> delta_kernels_;

  Value* excluded_region_;
  std::vector<Value*> extra_biases_;

  OFile stateOfile_;     // depends on bias
  int wStateStride_;
  bool storeOldStates_;

  bool use_tot_potential_;
  bool store_wavefunction_;
  bool call_qmprogram_flag_;

  double getProbAndDerivatives(const std::vector<double>&,std::vector<double>&);   //need to add new function
  ...
  void dumpStateToFile();        //need to add new function

public:
  explicit OPESmetad_dual2(const ActionOptions&);      //need to add new function
  void calculate() override;           //need to add new function
  void update() override;             //need to add new function?
  static void registerKeywords(Keywords& keys);                //need to add new function?
  void call_qmcode(bool&,bool&,std::array<double, 2>&);
  void print_bias(const double&);
  double read_bias();
};

//bias_mode: convergence_low_level (OPESmetad_c_low)= 1
//         convergence_high_level (OPESmetad_c_high)= 2
//         exploration_high_level (OPESmetad_e)= 3

struct convergence_low_level { static const int bias_mode=1; };
typedef OPESmetad_dual2<convergence_low_level> OPESmetad_c_low;
PLUMED_REGISTER_ACTION(OPESmetad_c_low,"DUAL_OPES_METAD_LOW_LEVEL")

struct convergence_high_level { static const int bias_mode=2; };
typedef OPESmetad_dual2<convergence_high_level> OPESmetad_c_high;
PLUMED_REGISTER_ACTION(OPESmetad_c_high,"DUAL_OPES_METAD_HIGH_LEVEL")

struct exploration_high_level { static const int bias_mode=3; };
typedef OPESmetad_dual2<exploration_high_level> OPESmetad_e;
PLUMED_REGISTER_ACTION(OPESmetad_e,"DUAL_OPES_METAD_EXPLORE_HIGH_LEVEL")

template <class mode>
void OPESmetad_dual2<mode>::registerKeywords(Keywords& keys)
{
...
//extra options
  ...
  keys.add("optional","PREFACTOR","a pre-factor before the bias potential");
...
//output total potential energy to H0H1 file
  keys.addFlag("USE_TOT_POTENTIAL",false,"use the total potential energy of H1 from MD program");
  keys.addFlag("STORE_WAVEFUNCTION",false,"store the wafvefunction of each high-level step");
  keys.addFlag("CALL_QMPROGRAM",false,"call the qm program to get high-level potential energy when depositing a new gaussian kernel");
...
//output components
  ...
  if(mode::bias_mode==2 || mode::bias_mode==3){
    keys.addOutputComponent("H0","default","the part potential energy of high-level(H0) used for reweighting");
    keys.addOutputComponent("H1","default","the part potential energy of low-level(H1) used for reweighting");
    keys.addOutputComponent("deltaH","default","potential energy difference of between low-level and high-level: H1-H0");
  }
}

template <class mode>
OPESmetad_dual2<mode>::OPESmetad_dual2(const ActionOptions& ao)
  : PLUMED_BIAS_INIT(ao)
  , isFirstStep_(true)
  , counter_(1)
...
//other compulsory input
  ...
  std::string prefactor_str;
  parse("BIASFACTOR",biasfactor_str);
  parse("PREFACTOR",prefactor_str);
  if(biasfactor_str=="inf" || biasfactor_str=="INF")
  {
    biasfactor_=std::numeric_limits<double>::infinity();
    plumed_massert(!std::isinf(biasfactor_),"BIASFACTOR=inf is not compatible with dual-level OPES metad");
  }
  else
  {
    ...
    plumed_massert(biasfactor_>0,"BIASFACTOR must be greater than 0 for dual-level OPES metad");
  }  
  if(prefactor_str.length()>0)
    plumed_massert(Tools::convertNoexcept(prefactor_str,prefactor_),error_in_input1+"BIASFACTOR"+error_in_input2);
  switch(mode::bias_mode)
  {
    default:
        bias_prefactor_= prefactor_*(1./biasfactor_);
        log.printf("  default BIAS_PREFACTOR = %f\n",bias_prefactor_);
        break;
    case 3:
        plumed_massert(!std::isinf(biasfactor_),"BIASFACTOR must be greater than 1 for dual-level OPES EXPLORE mode");
        bias_prefactor_= prefactor_*(biasfactor_-1);
        log.printf("  bias_mode 3: BIAS_PREFACTOR = %f\n",bias_prefactor_);
        break;
  }
  ...
  epsilon_=std::exp(-barrier/fabs(bias_prefactor_)/kbt_);
  ...
  double cutoff=sqrt(2.*barrier/fabs(bias_prefactor_)/kbt_);
  if(mode::bias_mode==3)
...
//write total potential energy to H0H1 file
  parseFlag("USE_TOT_POTENTIAL",use_tot_potential_);
//store the wavefunction of each high-level step
  parseFlag("STORE_WAVEFUNCTION",store_wavefunction_);
  parseFlag("CALL_QMPROGRAM",call_qmprogram_flag_);
//kernels file
  ...
  if(getRestart())
  {
    ...
    if(ifile.FileExist(restartFileName))
    {
      ...
      else
      {
        ...
        KDEnorm_=mode::bias_mode==3?counter_:sum_weights_;
        if(!no_Zed_)
        {
          double sum_uprob=0;
...
//add and set output components
  ...
  if(mode::bias_mode==2 || mode::bias_mode==3){
    addComponent("H0");
    componentIsNotPeriodic("H0");
    getPntrToComponent("H0")->set(hamiltonian_[0]);
    addComponent("H1");
    componentIsNotPeriodic("H1");
    getPntrToComponent("H1")->set(hamiltonian_[1]);
    addComponent("deltaH");
    componentIsNotPeriodic("deltaH");
    getPntrToComponent("deltaH")->set(deltaH_);
  }
...
//printing some info
  ...
  log.printf("  BIAS_PREFACTOR = %f\n",bias_prefactor_);
  log.printf("mode::bias_mode = %i\n",mode::bias_mode);
  if(adaptive_sigma_)
  {
...
//set bias and forces
  ...
  total_bias_ += bias;
  //print_bias(bias);
  // std::cout << "In the calculate function: current step is " << getStep() << " current bias:" << bias << std::endl;
}

template <class mode>
void OPESmetad_dual2<mode>::update()
{
...
//do update
  if(getStep()%stride_==0 && (excluded_region_==NULL || excluded_region_->get()==0))
  {
    ...
    // double log_weight=read_bias()/kbt_; // read the total bias from opes_bias_temp file
    double log_weight=total_bias_/kbt_;
    for(unsigned e=0; e<extra_biases_.size(); e++)
      log_weight+=extra_biases_[e]->get()/kbt_; //extra biases contribute to the weight
    if(mode::bias_mode==2 || mode::bias_mode==3){
      if(call_qmprogram_flag_){
        call_qmcode(use_tot_potential_,store_wavefunction_,hamiltonian_);
        // std::cout << "In update function: current step is" << getStep() << " call_qmcode" << std::endl;
      }
      if(!deltaH0_isAssigned){
        deltaH0=hamiltonian_[1]-hamiltonian_[0];
        deltaH0_isAssigned=true;
        deltaH_=0;
        // std::cout << "In update function: current step is" << getStep() << " assign the deltaH0, initialize the deltaH with 0" << std::endl;
      }
      else
      {
        deltaH_=hamiltonian_[1]-hamiltonian_[0]-deltaH0;
        // std::cout << "In update function: current step is" << getStep() << " update the deltaH" << std::endl;
      }
      log_weight+=deltaH_/kbt_; 
      // std::cout << "In update function: current step is" << getStep() << " the bias of current step:" << total_bias_ << " The deltaH value " << deltaH_ << std::endl;
    }
    ...
    if(adaptive_sigma_)
    {
      const double factor=mode::bias_mode==3?1:biasfactor_;
    ...
    if(!fixed_sigma_)
    {
      const double size=mode::bias_mode==3?counter_:neff; //for EXPLORE neff is not relevant
...
template <class mode>
double OPESmetad_dual2<mode>::getProbAndDerivatives(const std::vector<double>& cv,std::vector<double>& der_prob)
{
  double prob=0.0;
  if(!nlist_)
  {
...
template <class mode>
void OPESmetad_dual2<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma)
{
  bool no_match=true;
  if(threshold2_!=0)
  {
...
template <class mode>
void OPESmetad_dual2<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma,const double logweight)
{
  addKernel(height,center,sigma);
//write to file
...
template <class mode>
unsigned OPESmetad_dual2<mode>::getMergeableKernel(const std::vector<double>& giver_center,const unsigned giver_k)
...
template <class mode>
void OPESmetad_dual2<mode>::updateNlist(const std::vector<double>& new_center)
{
...
template <class mode>
void OPESmetad_dual2<mode>::dumpStateToFile()
{
...
template <class mode>
inline double OPESmetad_dual2<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x) const
...
template <class mode>
inline double OPESmetad_dual2<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x, std::vector<double>& acc_der, std::vector<double>& dist)
{ //NB: cannot be a method of kernel class, because uses external variables (for cutoff)
  double norm2=0;
  for(unsigned i=0; i<ncv_; i++)
  {
...
template <class mode>
inline void OPESmetad_dual2<mode>::mergeKernels(kernel& k1,const kernel& k2)
{
...
//a function to call the shell script to run the qm code 
//and get the potential energy of high-level & low-level Hamiltonian
template <class mode>
//void OPESmetad_dual2<mode>::call_qmcode(bool& use_tot_pot,bool& store_wavefunction_,std::array<double, 2>& hamil) {
//
//  int status;
//  int delete_result;
//  std::string command;
...
//    //}
//
//    // create the file CV_xy and write the cv value into the file
//    std::ofstream outFile(cv_file);
//    if (!outFile) {
//      plumed_massert(outFile.is_open(),"Unable to create file CV_xy");
//    }
//    //for (unsigned i=0; i < ncv_; i++) {
//    //    outFile << cv[i] << std::endl;
//    //}
//    plumed.getAtoms().getLocalPositions(positions);
//    outFile << positions[1][0] << std::endl;
//    outFile << positions[1][1] << std::endl;
//    outFile.close();
//
//    //call the C++ program calc_H0H1, which read the CV_xy to calculate the current H0 and H1
//    status = system("./calc_H0H1 >/dev/null 2>&1");
//    plumed_massert(status == 0,"Command failed with status"+std::to_string(status));
//  }
//
//  std::ifstream inputFile(ener_file);
//  plumed_massert(inputFile.is_open(),"Unable to open file H0H1");
//  if(!use_tot_pot){
//    for (int i = 0; i < 2; ++i) {
//      if (!(inputFile >> potential_energy[i])) {
//          inputFile.close(); 
//          plumed_merror("Error reading file H0H1: Expected 2 numbers, but couldn't read all.");
//      }
//    }
//  }
//  else
//  {
//      if (!(inputFile >> potential_energy[0])) {
//          inputFile.close(); 
//          plumed_merror("Error reading file H0H1: Expected 1 numbers, but couldn't read any.");
//      }
//      potential_energy[1] = plumed.getAtoms().getEnergy();
//  }
//  inputFile.close(); 
//
//  //delete the file H0H1 and CV_xy
//  if(mode::bias_mode!=2 || mode::bias_mode!=5){
//    delete_result = std::remove(ener_file);
//    plumed_massert(delete_result == 0,"Unable to delete file H0H1");
//    delete_result = std::remove(cv_file);
//    plumed_massert(delete_result == 0,"Unable to delete file CV_xy");
//  }
//
//  hamil[0]=potential_energy[0];
//  hamil[1]=potential_energy[1];
//}
void OPESmetad_dual2<mode>::call_qmcode(bool& use_tot_pot,bool& store_wavefunction_,std::array<double, 2>& hamil) {
  ...
  cal_H0H1_func(cv,potential_energy);
...
template <class mode>
void OPESmetad_dual2<mode>::print_bias(const double& current_bias) {
    const char* bias_file = "opes_bias_temp";

    std::ofstream outfile(bias_file, std::ios::app); // append the file with dual_OPESmetad bias. 
    // But in OPESmetad.cpp, the opes_bias_temp will be clean before writing new opes.bias  
    // So in the plumed.dat file, the OPESmetad block must be defined before dual_OPESmetad
     
    if (outfile.is_open()) {
        outfile << std::to_string(current_bias) << std::endl;
        outfile.close();
    } else {
        std::cerr << "Can not open the file opes_bias_temp!" << std::endl;
        outfile.close();
    }
...
template <class mode>
double OPESmetad_dual2<mode>::read_bias() {
    const char* bias_file = "opes_bias_temp";
    double tot_bias = 0.0;
    std::vector<double> bias_values;
    double temp;
    std::ifstream inputFile(bias_file);
    ...
    while (inputFile >> temp) {
        bias_values.push_back(temp);
    }
    
    if (inputFile.eof()) {
        inputFile.close();
        for (double element : bias_values) {
          tot_bias += element;
        }
        return tot_bias;
    } 
        ...
        inputFile.close();
        std::cerr << "Error when reading the file opes_bias_temp!" << std::endl;
    }
}
