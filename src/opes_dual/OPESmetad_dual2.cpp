...
//extra options
  ...
  keys.addFlag("USE_TOT_BIAS",false,"use total bias potential for reweighting KDE");
...
//options involving extra arguments
  ...
  parseFlag("USE_TOT_BIAS",use_total_bias);
...
//do update
  if(getStep()%stride_==0 && (excluded_region_==NULL || excluded_region_->get()==0))
  {
    ...
    if(!use_total_bias){
      double log_weight=getOutputQuantity(0)/kbt_; 
    }
    for(unsigned e=0; e<extra_biases_.size(); e++)
...
void OPESmetad_dual2<mode>::call_qmcode(bool& use_tot_pot,bool& store_wavefunction_,std::array<double, 2>& hamil) {
  ...
  //store the wavefunction of each high-level step if needed
  
  command="sed 's/step=md_step/step="+std::to_string(plumed.getStep())+"/' qm_shuttle.sh.pre > qm_shuttle.sh";
  system(command.c_str());
  command="chmod +x ./qm_shuttle.sh";
  system(command.c_str());
  //call the shell script, and redirect the output to  /dev/null
  if(mode::bias_mode!=3 || mode::bias_mode!=6){
    status = system("./qm_shuttle.sh >/dev/null 2>&1");
